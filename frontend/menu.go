...
func renderMenu(a *App, tab string) {
    ...
    if tab == "dashboard" {
        ...
        if a.User != nil {
            ...
            displayWPM = fmt.Sprintf("%.1f", a.User.AvgWpm)
        }
        ...
        cont = `
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                ...
                <div class="hud-border p-6 bg-black/40 backdrop-blur-md border-[#00f3ff]/20 text-right">
                    ...
                    <div class="text-5xl font-black tracking-tighter text-white">
                        <span id="display-wpm">` + displayWPM + `</span>
                        <span class="text-sm opacity-30 ml-2 text-[#00f3ff]">WPM</span>
                    </div>
                </div>
            ...
            <div class="relative z-10 mt-8">
                ...
                <div id="lobby-list" class="grid gap-4">
                    <div class="text-center opacity-30 text-xs py-10 animate-pulse font-mono">SCANNING_FREQUENCIES...</div>
                </div>
            </div>`

        js.Global().Set("joinLobby", js.FuncOf(func(this js.Value, args []js.Value) any {
            if len(args) > 0 { a.navigate("/lobby/" + args[0].String()) }
            return nil
        }))
    ...
    html := `
    <div class="fixed inset-0 flex bg-black/20 text-[#00f3ff] font-mono uppercase overflow-hidden">
        <div class="absolute inset-0 opacity-5 pointer-events-none" 
             style="background-image: radial-gradient(#00f3ff 1px, transparent 1px); background-size: 30px 30px;"></div>

        <div class="w-72 border-r border-[#00f3ff]/20 bg-black/60 backdrop-blur-xl p-8 flex flex-col z-20">
            <div class="mb-20">
                <div class="text-3xl font-bold glow-text tracking-tighter" style="font-family: 'Orbitron'; cursor: default;">
                    UP<span class="opacity-20">LINK</span>
                </div>
                <div class="text-[7px] tracking-[0.6em] opacity-30 mt-1">SECURE_TERMINAL_V4.0</div>
            </div>
        ...
        <div class="flex-1 p-12 overflow-y-auto relative z-10 flex flex-col items-center">
            <div class="w-full max-w-4xl">
                <header class="flex justify-between items-end mb-12 border-b border-[#00f3ff]/10 pb-4">
                    ...
                    <div class="flex flex-col items-end">
                        <div class="text-[9px] text-[#00f3ff] border border-[#00f3ff] px-3 py-1 mb-1 font-bold tracking-widest shadow-[0_0_10px_#00f3ff66]">
                            LINK_OK
                        </div>
                        <div class="text-[7px] opacity-20 uppercase font-mono">Encrypted connection active</div>
                    </div>
                </header>
...
func (a *App) updateMenuStats(rank int, wpm float64) {
	doc := js.Global().Get("document")

	rankEl := doc.Call("getElementById", "display-rank")
	if !rankEl.IsNull() {
		rankEl.Set("innerText", fmt.Sprintf("#%d", rank))
	}

	wpmEl := doc.Call("getElementById", "display-wpm")
	if !wpmEl.IsNull() {
		wpmEl.Set("innerText", fmt.Sprintf("%.1f", wpm))
	}
}